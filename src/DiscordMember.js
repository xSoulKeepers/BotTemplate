...
class DiscordMember {
  ...
  async prepareMember () {
    try {
      this.user = await this.bot.users[this.id]//.fetch(this.id, false)
      this.member = this.user//await this.server.members.fetch(this.user, false)
      return true
    } catch (e) {
  ...
  async verify (options) {
      ...
      status = action => {
        ...
        ;(async () => {
          ...
          if (typeof action === 'string') {
            statusMessage.edit(`${this.member}, ${action}`).then(() => statusMessage.delete(150000))
          } else if (action.error != null) {
            statusMessage.edit(`${this.member}, :exclamation:${action.error.startsWith(':') ? '' : ' '}${action.error}`).then(() => statusMessage.delete(150000))
            ...
            statusMessage.edit(`${options.message.author}, :white_check_mark: ${welcomeMessage}`).then(() => statusMessage.delete(150000))
          }
        })()
    ...
    if (!this.discordServer.isAuthorized()) {
      return status({
        status: false,
        error: "Sorry, We had a back-end error and Kyle is having some issues."
      })
    }
    ...
    if (this.member.bot) {
      return status({
        status: false,
        error: ':robot: Kyle cannot verify bots.'
      })
    }
    ...
    if (this.member.roles.find(role => role.name === 'RoVer Bypass')) {
      return status({
        status: false,
        error: ':octagonal_sign: Kyle cannot act on users with the "RoVer Bypass" role.',
        nonFatal: true
      })
    }
    ...
    if (!this.member.manageable) {
      errorAppend = "\n\nKyle's position in the role list is below that of this user. Please have a server admin drag Kyle's role above all other roles in order to fix this problem."
    }
    ...
    if (data.status === 'ok') {
      ...
      if (this.discordServer.getSetting('verifiedRole')) {
        ...
        if (!this.member.roles.has(role) && this.server.roles.has(role)) {
          try {
            await this.member.addRole(role)//.roles.add(role)
          } catch (e) {
            if (config.loud) console.log(e)
            return status({
              status: false,
              nonFatal: true,
              error: "There was an error while trying to assign the verified role! Ensure Kyle's role is above it." + errorAppend
            })
          }
        }
      }
        ...
        if (this.member.roles.has(role) && this.server.roles.has(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "There was an error while trying to remove the verified role! Ensure Kyle's role is above it." + errorAppend
            })
          }
        }
      }
        ...
        if (this.member.displayName !== nickname) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: this.member.guild.ownerID === this.member.id ? "Sorry, Discord doesn't allow bots to change the server owner's nickname. Please manually update your nickname. Or don't, I'm just an error message." : "Kyle doesn't have permission to change that user's nickname." + errorAppend
            })
          }
        }
      }
      ...
      if (this.discordServer.getSetting('groupRankBindings').length > 0) {
        ...
        try {
          ...
          return status({
            status: false,
            nonFatal: true,
            error: "Something went wrong when checking group membership. It appears the Roblox group API is offline or returning malformed data. It's possible Roblox is down for maintenance or there is something else wrong with Roblox. Please try again later."
          })
        }
      }
