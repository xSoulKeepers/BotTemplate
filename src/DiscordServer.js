...
class DiscordServer {
     ...
     * @param {DiscordBot} discordBot This discord bot that the server belongs to.
    ...
    constructor (discordBot, id) {
        ...
        this.server = this.bot.guilds[id]
        this.bot.guilds.forEach((guild, g) => { if (g == id) this.server = guild })
        ...
        // We will load the settings in the DiscordBot.getServer in order to know when
    ...
    async loadSettings () {
        ...
        try {
            let verifyChannelId = this.getSetting('verifyChannel')
            if (verifyChannelId) {
                let verifyChannel
                this.server.channels.forEach((channel,id) => {if (verifyChannelId == id) { verifyChannel = channel}})
                if (verifyChannel) {
                    let BotMessageExists
                    verifyChannel.fetchMessages()
                    ...
                    if (!BotMessageExists) {
                        let embed = new RichEmbed()
                            .setAuthor("Server Verification", this.bot.displayAvatarURL)
    ...
    /**
     * Get a user-data setting 
     * @param {Snowflake} id The user ID
     * @param {string} key The setting key
     * @returns {any} The setting value 
     * @memberof DiscordServer
     */
    async getUserSetting (id, key) {
        if (!this.areSettingsLoaded) {
            throw new Error(`Attempt to get a setting from a server whose settings are not loaded`)
        }
        if (!this.settings.userData[id]) this.settings.userData[id] = []
        
        return this.settings.userData[id][key]
    }

    /**
     * Set a user-data setting and then save it to disk
     * @param {Snowflake} UserId Id
     * @param {string} key The setting key to set
     * @param {any} value The setting value to set
     * @memberof DiscordServer 
     */
    async setUserSetting (id, key, value) {
        if (!this.areSettingsLoaded) {
            throw new Error(`Attempt to change a setting from a server whose settings are not loaded`)
        }
        this.ongoingSettingsUpdate = true
        ...
        if (!this.settings.userData[id]) this.settings.userData[id] = []
        this.settings.userData[id][key] = value
     ...
     * @memberof DiscordServer 
     */
    async clearUserSetting (id) {
        if (!this.areSettingsLoaded) {
            throw new Error(`Attempt to clear a setting from a server whose settings are not loaded`)
        }
        this.ongoingSettingsUpdate = true
        if (this.settings.userData[id]) this.settings.userData[id] = null
        this.WriteData()
    }

    /**
     * Converts a group of binding from the old format to the new format.
     * @static
    ...
    async cleanupRankBindings (lastResortChannel) {
        ...
        let unverifiedInstance

        this.server.roles.forEach((r) => { if (r.id == verifiedRole) verifiedInstance = r})
        this.server.roles.forEach((r) => { if (r.id == unverifiedRole) unverifiedInstance = r})

        if (verifiedRole && !verifiedInstance) {
        ...
        if (unverifiedRole && !unverifiedInstance) {
    ...
    async announce (title, text, options = {}) {
        ...
        if (this.server.systemChannel) {
            try {
            await this.server.systemChannel.send(`${this.server.owner} An important notice was triggered and there is no announcement channel configured and the server owner will not accept DMs from Kyle, so it has been posted here:`, { embed })
            return
            } catch (e) {}
        }
        ...
        if (options.lastResortChannel) {
            try {
            await options.lastResortChannel.send(`${this.server.owner} An important notice was triggered and there is no announcement channel configured and the server owner will not accept DMs from Kyle, so it has been posted here as a last resort:`, { embed })
            } catch (e) {}
        }
        }
    }
